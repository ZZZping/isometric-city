...
export const Sidebar = React.memo(function Sidebar({ onExit }: { onExit?: () => void }) {
  ...
  // Zoning category (shown under ZONES section)
  const zoningCategory = useMemo(() => ({
    key: 'zoning',
    label: 'Zoning',
        ...
        {Object.entries(directCategories).map(([category, tools]) => (
          <div key={category} className="mb-1">
            ...
            <div className="px-2 flex flex-col gap-0.5">
              ...
              {/* Zoning submenu appears under ZONES section */}
              {category === 'ZONES' && (
                <HoverSubmenu
                  key={zoningCategory.key}
                  label={zoningCategory.label}
                  tools={zoningCategory.tools}
