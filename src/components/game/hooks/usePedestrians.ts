...
import { Pedestrian, CarDirection, WorldRenderState } from '../types';
import { PEDESTRIAN_SKIN_COLORS, PEDESTRIAN_SHIRT_COLORS, PEDESTRIAN_MIN_ZOOM } from '../constants';
import { isRoadTile, findPathOnRoads, getDirectionToTile } from '../utils';
...
import { TILE_WIDTH, TILE_HEIGHT, PedestrianDestType } from '../types';

export type UsePedestriansOptions = {
...
export function usePedestrians({ worldStateRef, gridVersionRef, cachedRoadTileCountRef, isMobile }: UsePedestriansOptions) {
  ...
  }, [worldStateRef, gridVersionRef, cachedRoadTileCountRef, spawnPedestrian, isMobile]);
