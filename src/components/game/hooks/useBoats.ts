...
import { Boat, TourWaypoint, WorldRenderState } from '../types';
import { BOAT_COLORS, BOAT_MIN_ZOOM, WAKE_MAX_AGE, WAKE_SPAWN_INTERVAL } from '../constants';
import { gridToScreen } from '../utils';
import { findMarinasAndPiers, findAdjacentWaterTile, isOverWater, generateTourWaypoints } from '../gridFinders';
import { TILE_WIDTH, TILE_HEIGHT } from '../types';

export type UseBoatsOptions = {
  worldStateRef: React.MutableRefObject<WorldRenderState>;
  isMobile: boolean;
};

export function useBoats({ worldStateRef, isMobile }: UseBoatsOptions) {
  ...
  const drawBoats = useCallback((ctx: CanvasRenderingContext2D, hour: number) => {
